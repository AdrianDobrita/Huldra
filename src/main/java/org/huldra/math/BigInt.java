...
public class BigInt extends Number implements Comparable<BigInt> {
     ...
     *
     * @param other The BigInt to copy/assign to this BigInt.
     * @complexity O(n)
     */
     ...
     * @throws Exception thread related exceptions.
     * @complexity O(n ^ 1.585)
     */
     ...
     * @param div
     */
    public void mod(final BigInt div) throws ArithmeticException {
     ...
     * @complexity O(m * n)
     */
    private void div(final int[] u, final int[] v, final int m, final int n, final int[] q) {//Hacker'sDelight's implementation of Knuth's Algorithm D
    ...
    public void and(final BigInt mask) {
        if (sign > 0) {
            if (mask.sign > 0) {
                ...
                if (a != 0 && b == 0) {
                    for (dig[j - 1] = 0; j < mlen && mask.dig[j] == 0; j++) {
                        dig[j] = 0;
                    }
                    if (j < mlen) {
                        dig[j] &= -mask.dig[j];
    ...
    public void xor(final BigInt mask) {
        if (sign > 0) {
            ...
            if (mask.sign > 0) {
                ...
                {
                    for (dig[j - 1] = -b; j < mask.len && dig[j] == 0; j++) {
                        dig[j] = ~mask.dig[j];
                    }
                    while (dig[j] == 0) {
                        dig[j++] = -1;
        ...
        if (sign > 0) {
            ...
            if (mask.sign > 0) {
                ...
                } else if (a == 0) // && (b!=0 || j==mlen)
                {
                    for (; j < mlen && dig[j] == 0; j++) ;
                    if (j < mlen) {
                        dig[j] = -dig[j] & mask.dig[j]; // ~~mask.dig[j]
                    }
                    ++j;
                } else {
                    dig[j - 1] = -a & ~-b;
